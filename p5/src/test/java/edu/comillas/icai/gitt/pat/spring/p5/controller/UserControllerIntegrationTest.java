...
class UserControllerIntegrationTest {
    ...
    void registerInvalidPassword() throws Exception {
        // Given: se prepara una solicitud con una contraseña insegura (por ejemplo, solo números)
        String request = "{" +
                "\"name\":\"" + NAME + "\"," +
                "\"email\":\"" + EMAIL + "\"," +
                "\"role\":\"" + Role.USER + "\"," +
                "\"password\":\"123456\"}"; // Contraseña inválida: no tiene letras, solo números

        // When: se realiza la solicitud POST al endpoint de registro
        this.mockMvc
                .perform(MockMvcRequestBuilders.post("/api/users")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(request))
                // Then: se verifica que la respuesta sea un error 400 (Bad Request)
                .andExpect(MockMvcResultMatchers.status().isBadRequest())  // Se espera un código 400
                .andExpect(MockMvcResultMatchers.content().string(""));  //
    }
